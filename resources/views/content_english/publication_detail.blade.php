...
@section('title')
    {{ $publikasi->judul_english ?? $publikasi->judul_indo }} - Jalur Rempah Kemdikbudristek Republik Indonesia
@endsection
            ...
            <section id="artikel">
                ...
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <header>
                                <h2 class="sub-judul">{{ $publikasi->judul_english }}</h2>
                                <div class="info-penulis">
                                    <span class="txt-penulis" class="mr-3" id="penulis"
                                        name="penulis">{{ $publikasi->penulis != 'admin' ? $publikasi->kontributor_relasi->nama : 'admin' }}</span>
                                    |
                                    <span class="txt-penulis" id="tglArtikel"
                                        name="tglArtikel">{{ \Carbon\Carbon::parse($publikasi->published_at)->isoFormat('D MMMM Y') }}</span>
                                </div>
                            </header>
                            <section id="desTentang">
                                <img class="mb-3 mt-3"
                                    src="{{ asset('storage/assets/publikasi/thumbnail/' . $publikasi->thumbnail) }}"
                                    width="100%">
                            ...
                            @if ($publikasi->penulis != 'admin')
                                <div id="disclaimer" class="mt-4">
                                    <p>Konten ini dibuat oleh kontributor website Jalur Rempah. <br>
                                        ...
                                        tanggung jawab kontributor sepenuhnya.</p>
                                </div>
                            @endif
                            @php
                                            ...
                                            <div class="col-12 mb-2">
                                                <div class="card no-border no-background">
                                                    <div class="card-body">
                                                        <div class="row">
                                                            <div class="col-5">
                                                                <img class="kegiatan-img" id="imgKegiatan"
                                                                    name="imgKegiatan"
                                                                    src="{{ asset('storage/assets/publikasi/thumbnail/' . $a->thumbnail) }}">
                                                            </div>
                                                            <div class="col-7 center-v">
                                                                <h3 class="judul-berita-aside" id="jdlKegiatan"
                                                                    name="jdlKegiatan">{{ $a->judul_english }}</h3>
                                                        ...
                                                        <a href="{{ route('publication_detail', $a->slug_english ?? $a->slug) }}"
            ...
            <section class="container mt-5">
                <header class="row justify-content-center mb-2">
                    <div class="col-md-6">
                        ...
                        <p>Berbagai upaya dilakukan untuk melestarikan jalur rempah, salah satunya dengan melakukan berbagai
                            kegiatan.</p>
                    </div>
                    ...
                    @foreach ($publikasiPopuler as $a)
                        <div class="col-md-6 col-lg-4 mb-4">
                            <div class="card no-border no-background card-body">
                                <img src="{{ asset('storage/assets/publikasi/thumbnail/' . $a->thumbnail) }}"
                                ...
                                <p class="penulis-artikel">
                                    {{ $a->penulis != 'admin' ? $a->kontributor_relasi->nama : 'admin' }}</p>
                                <p class="tgl-artikel">
                                    {{ \Carbon\Carbon::parse($a->published_at)->isoFormat('D MMMM Y') }}</p>
                                <a href="{{ route('publication_detail', $a->slug_english ?? $a->slug) }}"
                                    class="stretched-link"></a>
                            </div>
                        </div>
